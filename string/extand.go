...
func (e extend) Sub(src string, start int, length ...int) string {
	charList := []rune(src)
	l := len(charList)
	step := 0
	end := 0
...
// Len 返回char字符串长度
func (e extend) Len(src string) int {
	return len([]rune(src))
}

// Before 获取某个字符串第一个出现位置之前的字符串，如果不存在返回源字符串
func (e extend) Before(src string, target string) string {
	if target == "" {
		return src
	}
...
func (e extend) BeforeLast(src string, target string) string {
	if target == "" {
		return src
	}
...
func (e extend) After(src string, target string) string {
	if target == "" {
		return src
	}
	i := strings.Index(src, target)
...
func (e extend) AfterLast(src string, target string) string {
	if target == "" {
		return src
	}
	i := strings.LastIndex(src, target)
