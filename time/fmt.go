...
func (tf tFmt) LayoutDateTime() string {
	return "2006-01-02 15:04:05"
}

// LayoutDate 返回日期格式
func (tf tFmt) LayoutDate() string {
	return "2006-01-02"
}

// LayoutTime 返回时间格式
func (tf tFmt) LayoutTime() string {
	return "15:04:05"
}

func (tf tFmt) CTSZone() *time.Location {
	return time.FixedZone("CST", 8*3600)
}

// GetNowDateTime 获取当前日期时间字符串
func (tf tFmt) GetNowDateTime() string {
	return time.Now().In(tf.CTSZone()).Format(tf.LayoutDateTime())
}

// GetNowDate 获取当前日期字符串
func (tf tFmt) GetNowDate() string {
	return time.Now().In(tf.CTSZone()).Format(tf.LayoutDate())
}

// GetNowTime 获取当前时间字符串
func (tf tFmt) GetNowTime() string {
	return time.Now().In(tf.CTSZone()).Format(tf.LayoutTime())
}

// GetNowWeekday 获取当前星期字符串
func (tf tFmt) GetNowWeekday() string {
...
// ss - second - 05
// s - second = 5
func (tf tFmt) Format(t time.Time, format string) string {
...
func (tf tFmt) Convert(timeString string, format string) string {
	t, err := dateparse.ParseLocal(timeString)
	if err != nil {
		fmt.Println("ParseLocal error:", err.Error())
		return ""
	}
	return tf.Format(t, format)
}
